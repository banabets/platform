...
export const Screen = styled.div<{ $isRoulette?: boolean }>`
  position: relative;
  flex-grow: 1;
  background: ${props => props.$isRoulette 
    ? 'linear-gradient(145deg, #0B5E2E, #0A4D25)' 
    : '#0c0c11'};
  border-radius: 10px;
  overflow: hidden;
  ...
  /* Textura de fieltro para ruleta */
  ${props => props.$isRoulette && css`
    background-image: 
      linear-gradient(145deg, #0B5E2E, #0A4D25),
