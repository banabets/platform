...
export default function Game() {
  ...
  const { trackGameStart, trackError } = useAnalytics()

  // Track cuando el usuario entra a un juego
  React.useEffect(() => {
    if (game) {
  ...
  // Track errores del juego
  React.useEffect(() => {
    if (!game) {
      trackError('game_not_found', `gameId: ${gameId}`)
    }
  }, [game, gameId, trackError])
